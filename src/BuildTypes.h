...
struct DataTypeInfo
{
    ...
    DataTypeInfo(const char *name, unsigned bitWidth, Type *llvmType, unsigned dwarfEnc) : name(name), bitWidth(bitWidth), llvmType(llvmType), dwarfEnc(dwarfEnc), diType(NULL),
...
public:
    ...
    bool isTemplateType(const DataType &dt) const
    {
        // Evita crash se for indefinido
        if (dt == undefinedType)
        {
            return true;
        }
