...
// Array::Array(const string& n, ArrayElements *aes, location_t loc) : Cloneable<Array, Variable>(n, loc), arrayElements(aes) {
// 	NamedConst *nc = new NamedConst("size", getNodeForUIntConst(aes->getArraySize(), loc));
// 	addChild(nc);
// 	addSymbol(nc);
// }

// Array::Array(const string& n, location_t loc) : Cloneable<Array, Variable>(n, loc) {
// 	arrayElements = new ArrayElements(getLoc());
// }

// Array::Array(const Array& other, TypeSubs& ts)
//     : Cloneable<Array, Variable>(other, ts)
// {

// 	arrayElements->getElements().clear();

// 	for (ArrayElement* ae : other.arrayElements->getElements())
//     {
//         // 1) clonar o value
//         Node* clonedValue = nullptr;
//         if (ae->value)
//             clonedValue = ae->value->cloneTree(ts);

//         // 2) copiar o count
//         unsigned int newCount = ae->count;

//         // 3) criar novo elemento
//         ArrayElement* newAE = new ArrayElement(clonedValue, newCount);

//         // 4) adicionar ao vetor
// 		arrayElements->getElements().push_back(newAE);

//         // 5) adicionar como filho do Node
//         this->addChild(clonedValue);
//     }

//     for (Node* child : other.children())
//     {
//         // size é NamedConst
//         if (NamedConst* nc = dynamic_cast<NamedConst*>(child))
//         {
//             NamedConst* cloned = new NamedConst(*nc, ts);
//             addChild(cloned);
//             addSymbol(cloned);
//         }
//     }
