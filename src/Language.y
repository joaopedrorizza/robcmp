...
expr : expr TOK_OR expr		{ $$ = new BinaryOp($1, TOK_OR, $3); }
expr : expr TOK_AND expr	{ $$ = new BinaryOp($1, TOK_AND, $3); }
expr : expr EQ_OP expr		{ $$ = new CmpOp($1, EQ_OP, $3); }
expr : expr NE_OP expr		{ $$ = new CmpOp($1, NE_OP, $3); }
expr : expr LE_OP expr		{ $$ = new CmpOp($1, LE_OP, $3); }
expr : expr GE_OP expr		{ $$ = new CmpOp($1, GE_OP, $3); }
expr : expr '<' expr		{ $$ = new CmpOp($1, LT_OP, $3); }
expr : expr '>' expr		{ $$ = new CmpOp($1, GT_OP, $3); }
expr : expr '+' expr			{ $$ = new BinaryOp($1, '+', $3); }
expr : expr '-' expr			{ $$ = new BinaryOp($1, '-', $3); }
expr : expr '|' expr			{ $$ = new BinaryOp($1, '|', $3); }
expr : expr TOK_LSHIFT expr		{ $$ = new BinaryOp($1, TOK_LSHIFT, $3); }
expr : expr TOK_RSHIFT expr		{ $$ = new BinaryOp($1, TOK_RSHIFT, $3); }
expr : expr '*' expr		{ $$ = new BinaryOp($1, '*', $3); }
expr : expr '/' expr		{ $$ = new BinaryOp($1, '/', $3); }
expr : expr '%' expr		{ $$ = new BinaryOp($1, '%', $3); }
expr : expr '^' expr		{ $$ = new BinaryOp($1, '^', $3); }
expr : expr '&' expr    	{ $$ = new BinaryOp($1, '&', $3); }
expr : '-' expr[e]			{ $$ = new BinaryOp($e, '*', getNodeForIntConst(-1, @e)); }
expr : '~' expr[e]			{ $$ = new FlipOp($e); }
expr : '!' expr[e]			{ $$ = new BinaryOp(new Int1(1, @e), '-', $e); }
expr : factor	{ $$ = $1; }
...
call_or_cast : ident_or_xident[id] '(' paramscall ')' {
	...
	else if (strncmp("sizeof", $id, 6) == 0 && $paramscall->getNumParams() == 1)
		$$ = new Sizeof($paramscall->getParamElement(0), @id);
